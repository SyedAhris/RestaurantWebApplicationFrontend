...
function EmployeesOfOnlineOrders() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch('http://localhost:3001/getemployeesonlineorderview')
    ...
    const handleSubmit = (event) => {
        ...
        if (form.checkValidity() === false) {
          ...
          event.stopPropagation();
            ...
            Axios.post("http://localhost:3001/addonlineorderhasemployees", {
                orderID: addOrdID,
                empID: addEmpID,
    ...
    const deletee = (event) => {
        ...
        const ordID = employeesTable[event.currentTarget.id].onlineorder_id;
        const empID = employeesTable[event.currentTarget.id].employee_id;
        console.log( ordID + " " + empID);
        Axios.post("http://localhost:3001/deleteonlineorderhasemployees", {
            orderID: ordID,
            empID: empID,
                    ...
                    <tbody>
                        {employeesTable.map((employeesTable, k) => (
                         <tr id={employeesTable.emp_id,employeesTable.order_id} key={k}>
                            <td>{employeesTable.onlineorder_id}</td>
                            <td>{employeesTable.employee_id}</td>
...
export default EmployeesOfOnlineOrders
